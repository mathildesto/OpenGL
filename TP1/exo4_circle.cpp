...
// Création d'un seul VBO:
        ...
        for (int k = 0; k<N; k++){
            v.push_back(Vertex2DColor(glm::vec2(0.0, 0.0), glm::vec3(1, 0, 0)));
            v.push_back(Vertex2DColor(glm::vec2(glm::cos(k*2*glm::pi<float>()/N), glm::sin(k*2*glm::pi<float>()/N)), glm::vec3(0, 1, 0)));
            v.push_back(Vertex2DColor(glm::vec2(glm::cos((k+1)*2*glm::pi<float>()/N), glm::sin((k+1)*2*glm::pi<float>()/N)), glm::vec3(0, 1, 0)));
        }

//             Vertex2DColor vertices[] = { 
//             Vertex2DColor(glm::vec2(0.0, 0.0), glm::vec3(1, 0, 0)),
//             Vertex2DColor(glm::vec2(glm::cos(k*2*glm::pi<float>()/N), glm::sin(k*2*glm::pi<float>()/N)), glm::vec3(0, 1, 0)),
//             Vertex2DColor(glm::vec2(glm::cos((k+1)*2*glm::pi<float>()/N), glm::sin((k+1)*2*glm::pi<float>()/N)), glm::vec3(0, 0, 1))
// };
        ...
        glBufferData(GL_ARRAY_BUFFER, 3*N*sizeof(Vertex2DColor), v.data(), GL_STATIC_DRAW);
